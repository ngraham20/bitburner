...
function advanced_calculate_optimal_targets(ns, targets, topx) {
    let servers = targets.filter(s => weight(ns, s) > 0);
    servers.sort((a, b) => weight(ns, b) - weight(ns, a));
    return servers.slice(0,topx)
}
...
function parse_orders(ns, threadTargets) {
    ...
    if (targets[0] == "NULL PORT DATA") {
        return advanced_calculate_optimal_targets(ns, threadTargets, 5);
    }
    if (targets[0] == "top") {
        topx = targets[1];
        return advanced_calculate_optimal_targets(ns, threadTargets, topx);
    }
    return advanced_calculate_optimal_targets(ns, targets, targets.length);
}
